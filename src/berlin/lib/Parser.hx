package berlin.lib;
import berlin.lib.Exp.ExpType;
import berlin.lib.Util.Assert;
...
class Parser
{
	...
	public function match(nt:String, input:String):Dynamic
	{
		...
		var move:MachineConfiguration->MachineState = function (conf:MachineConfiguration):MachineState
		{
			...
			var kRest:Array<Dynamic> = new Array<Continuations>();
		...
		var state:MachineState = move(MachineConfiguration.EVAL(cast(this.env.get(nt), NonTerminal).exp, 0, [], new RawError(0, [nt], [], nt), []));
